...
function getDependencies(config, exports, callback) {
  writeExports(exports, function(err, exportsPath) {
    ...
    log.info('ol3-panzoom', 'Parsing dependencies');
    var options;
    if (config.src) {
      options = {
        lib: config.src,
...
function build(config, paths, callback) {
  ...
  if (!options.compile) {
    log.info('ol3-panzoom', 'No compile options found.  Concatenating ' +
        paths.length + ' sources');
    concatenate(paths, callback);
  } else {
    log.info('ol3-panzoom', 'Compiling ' + paths.length + ' sources');
...
function addHeader(compiledSource, callback) {
  exec('git describe --tags', function(error, stdout, stderr) {
    var header = '// Ol3-PanZoom. See https://github.com/mapgears/ol3-panzoom/\n';
    header += '// License: https://github.com/mapgears/ol3-panzoom/blob/master/LICENSE\n';
    if (stdout !== '') {
